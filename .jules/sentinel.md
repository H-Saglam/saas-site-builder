...
**Vulnerability:** The cron authorization header was being compared using the standard `===` operator.
...
**Prevention:** Always use `safeCompare` (which utilizes `crypto.timingSafeEqual` after hashing inputs to normalize lengths) for comparing sensitive secrets, such as authorization headers or tokens.
