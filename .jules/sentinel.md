...
## 2024-05-25 - [Timing Attack in Auth Headers]
**Vulnerability:** Comparing `authorization` headers directly with standard `===` strings allows an attacker to perform timing attacks, discovering the secret over time based on comparison duration.
**Learning:** Node.js V8 string comparison operations optimize aggressively, exiting early upon mismatched characters. This behavior leaks secret characters.
**Prevention:** Always use `safeCompare` (which leverages `crypto.timingSafeEqual`) for comparing sensitive strings, especially for authentication and authorization.
